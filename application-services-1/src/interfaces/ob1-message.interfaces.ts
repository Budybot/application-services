...
export namespace OB1Global {
  ...
  export class MessageHeaderV2 {
    ...
    @Matches(/^0\.\d+\.\d+$/, {
      message:
        'schemaVersion must match the semantic versioning format (e.g., 0.1.6).',
    })
...
export function validateSchemaVersion(header: OB1Global.MessageHeaderV2): {
  valid: boolean;
  errorMessage?: string;
} {
...
export async function validateOutgoingMessageHeader(
  ...
  try {
    const headerInstance = plainToInstance(
...
export function generateDefaultErrorMessageResponseValue(
  ...
  errorCode: number;
  errorMessage: string;
  messageContent: { errorDetails: any };
} {
  return {
    error: true,
    errorCode,
